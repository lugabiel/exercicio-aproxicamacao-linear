#--------         CODIGO POR GABRIEL ARAGAO           ----------
#ESTE CODIGO EH LIVRE E PODE SER COPIADO, EDITADO E MODIFICADO.
...
for (j in 1:10) {
  ...
  rm(list=ls())       #realiza limpeza do espaÃ§o de variaveis
  ...
  #boxplot(matrix_resultado,main='Erro Quadrático Médio (E.Q.M.) vs Complexidade', col='lightblue',xlab='Ordem de complexidade modelo',ylab='E.Q.M. em relação a p(x)')
  ...
  #variancia_total[j]<- c(variancia_total,variancia)
  
  plot(variancia, main='Variância do Erro vs Complexidade',xlab='Ordem de complexidade modelo',ylab='Variância do Erro em relação a p(x)')
